...
the digit used for XOR operation by different clients is different
...
def get_xor_code(sample_file: str) -> int:
    """
    Figure out the digit used to do XOR operation
...
def decrypt_file(src_file: str, dst_file: str, xor_code: int) -> None:
    """
    Decrypt a single file
    :param src_file: the encrypted .dat file
    :param dst_file: the decrypted .jpg / .png file
    :param xor_code: the digit used in XOR operation
    :return: None
    """
    new_byte = list()
...
def main(src: str, dst: str) -> int:
    ...
    for file in tqdm(files):
        decrypt_file(src + '/' + file, dst + '/' + file, xor_code)
    return len(files)
